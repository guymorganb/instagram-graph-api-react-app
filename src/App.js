  ...
  const logInToFB = () => {
    window.FB.login(
      ...
      },
      {
        ...
        scope: "instagram_basic,pages_show_list,manage_pages,instagram_content_publish",
        ...
        window.FB.api(
          ...
          (response) => {
            if (response && response.data) {
              ...
              reject(response);  // Capture the entire response
            }
          }
        );
      });
    ...
    const shareInstagramPost = async () => {
      ...
      try {
        ...
        if (facebookPages && facebookPages.length > 0) {
          ...
          console.error("No Facebook pages found for the user.");
        }
      } catch (error) {
        console.error("Error sharing the Instagram post:", error);
      }
